...
def add_arange_ids(data_list):
    """
    Add the index as an id to the edge features to find seed edges in
    ...
    for data in data_list:
        if isinstance(data, HeteroData):
            data["node", "to", "node"].edge_attr = torch.cat(
                [
                    torch.arange(data["node", "to", "node"].edge_attr.shape[0]).view(-1, 1),
            ...
            data["node", "rev_to", "node"].edge_attr = torch.cat(
                [
                    torch.arange(offset, data["node", "rev_to", "node"].edge_attr.shape[0] + offset).view(-1, 1),
