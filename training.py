...
def get_model(sample_batch, config, args):
    ...
    e_dim = \
        (sample_batch.edge_attr.shape[1] - 1) \
        if not isinstance(sample_batch, HeteroData) \
        else (sample_batch['node', 'to', 'node'].edge_attr.shape[1] - 1)
    ...
    if args.model == "gin":
        ...
        if not isinstance(sample_batch, HeteroData):
            ...
            index = torch.cat(
                [
                    sample_batch['node', 'to', 'node'].edge_index[1],
                    sample_batch['node', 'rev_to', 'node'].edge_index[1],
                ],
                dim=0
            )
