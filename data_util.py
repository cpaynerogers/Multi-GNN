...
def to_adj_nodes_with_times(data):
    ...
    edges = \
        torch.cat((data.edge_index.T, timestamps), dim=1) \
        if not isinstance(data, HeteroData) \
        else torch.cat((data["node", "to", "node"].edge_index.T, timestamps), dim=1)
...
def to_adj_edges_with_times(data):
    ...
    timestamps = \
        ...
        else data.timestamps.reshape((-1,1))
